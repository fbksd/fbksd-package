...
int main(int argc, char **argv)
{
    ...
    {
        auto initialSampler = BandwidthSampler(&sampler, 0, width, 0, height);
        RNG rng;
        int nSamples = 0;
        size_t nSum = 0;
        while((nSamples = initialSampler.GetMoreSamples(&samples[nSum*sampleSize], rng)) > 0)
            nSum += nSamples;
        std::cout << nSum << " initial samples generated" << std::endl;
    }
