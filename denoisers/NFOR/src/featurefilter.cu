...
void FeatureFilter::PushGuide(float* mean, float* mean_var, int nchannels) {
    if (data == NULL) {
        MallocData(1);
    }
    ...
    size_t size_img_bytes = (nchannels_guide == 1) ? size_img_bytes_1c : size_img_bytes_3c;
    CUDA_CHECK_RETURN(cudaMemcpy(data_mean, mean, size_img_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK_RETURN(cudaMemcpy(data_mean_var_num, mean_var, size_img_bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK_RETURN(cudaMemcpy(data_mean_var_den, mean_var, size_img_bytes, cudaMemcpyHostToDevice));
}
...
void FeatureFilter::PushData(float* buffer, int nchannels) {
    ...
    CUDA_CHECK_RETURN(cudaMemcpy(data, buffer, size_img_bytes, cudaMemcpyHostToDevice));
