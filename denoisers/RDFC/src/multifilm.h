...
struct StatsInfo {
    void Init(int npixels) {
        ...
        position_mean_var.resize(3*npixels);
        visibility_mean_var.resize(npixels);
        caustic_mean_var.resize(3*npixels);
    ...
    vector<float> caustic_mean;
    vector<float> position_mean;
    vector<float> visibility_mean;
    // second moment
    vector<float> rgb_m2;
    vector<float> normal_m2;
    vector<float> albedo_m2;
    vector<float> caustic_m2;
    vector<float> position_m2;
    ...
    vector<float> caustic_mean_var;
    vector<float> position_mean_var;
...
public:
    ...
    MultiFilm(int xres, int yres, Filter *filt, int wnd_rad);
    ~MultiFilm() {
//        delete filter;
    }
    ...
    void Splat(const CameraSample &sample, const Spectrum &L) {
        Error("MultiFilm::Splat: Not supported!");
    }
    void AddSample(const float* sample) {
        Error("MultiFilm::AddSample: Not supported!");
    }
